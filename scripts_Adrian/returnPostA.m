function ss=returnPostA(lTrain, rTrain, rateLow, rateHigh, T, gammax,...
...
%        gammax-by-posttimes containing the 
%        values of the posterior over change points count

ss=zeros(gammax,length(posttimes));
coliter=1;
...
while time<T
    ...
    if t_new > nxtposttime
        ...
        xp=yp_new-K;
        xm=ym_new-K;
        marginalOverGamma=exp(xp)+exp(xm);
        %disp(t_new)
        %disp(sum(marginalOverGamma))
        ss(:,coliter)=marginalOverGamma;
        coliter=coliter+1;
